        ...
        <>
                ...
                <div className="row">
                    ...
                    <div className="col-xs-6 col-sm-6 col-md-4 col-lg-3 product1  bg-light" key={index}>
                        ...
                        <div className="m-auto proimg" style={{maxWidth:'180px',maxHeight:'250px'}}><Link to={`/productDetails?pid=${item._id}`}><img src={ item.images} alt="book" className="img-fluid" style={{width:'100%',height:'100%'}}/></Link><i className={`wishicon fs-1 fw-semibold `} style={{}} onClick={()=>WishList(item._id)}>< IoIosHeartEmpty fill={likedItems.includes(item._id)? "red" : ""} /></i></div>
                            
                            <Link className="" style={{textDecoration:'none'}} to={`/productDetails?pid=${item._id}`}><p className="mb-1 text-success">{item.tittle}</p></Link>
                            <div className="d-flex gap-2 my-2"><div className="text-muted text-decoration-line-through ">&#8377;350</div><div className="bg-success text-white px-2 rounded-pill text-center" style={{fontSize:'smaller',alignItems:'center',paddingTop:'2px'}}>10% Off</div><div className="fw-bold">&#8377;{item.price}</div></div>
