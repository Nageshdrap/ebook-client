...
                </div>
  ...
  {wishlist.length > 0 && <span className="wishlist-badge"></span>}
                ...
                <div className='position-relative' style={{cursor:'pointer'}} onClick={() => setCartOpen(true)}><IoCartOutline className='mx-2  fw-semibold' style={{fontSize:'1.5rem'}} />{ cartItems && <span className='position-absolute   translate-middle qunti px-2  bg-danger border border-light rounded-circle text-white'>{cartItems.length}</span>}</div>
